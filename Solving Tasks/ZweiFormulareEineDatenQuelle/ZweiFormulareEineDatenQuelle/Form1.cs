...
namespace ZweiFormulareEineDatenQuelle
{
  public partial class Form1 : Form
  {
    ...
    //---> DataSet, DataAdapter, DataTable, DataView, CommandBuilder:
    // • DataSet
    ...
    {
      dataRowView = dataView[dataGridView1.CurrentRow.Index + 1];
    ...
    private void buttonLöschen_Click(object sender, EventArgs e)
    {
      if (dataView.Count > 0)
      {
        var collegue = dataView[dataGridView1.CurrentRow.Index+1]["Kontaktperson"];
        string msg = "Wollen Sie den Mitarbeiter " + collegue.ToString() + " wirklich löschen?";
        ...
        if (MessageBox.Show(msg, cpt, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
          dataView[dataGridView1.CurrentRow.Index+1].Delete();
        buttonSpeichern.Enabled = true;
      }
      else
    ...
    /// <summary> gets all tables and their schemes(rows and their data types)
    /// </summary>
    /// <param name="sender"></param>
    /// <param name="e"></param>
